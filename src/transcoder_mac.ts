...
async function transcodeResolutions() {
    ...
    for (const resolution of resolutions) {
        ...
        try {
            ...
            logWithTimestamp(`Failed to transcode ${resolution.name}: ${error}`, chalk.red);
        }
    }
}
...
async function copySubtitles() {
    ...
    try {
        ...
        subtitleProgress.complete(`Processed ${embeddedSubtitles.length} subtitle tracks`);
    } catch (error) {
...
async function writeMasterPlaylist() {
    ...
    if (audioTracks.length > 0) {
        ...
        for (const track of audioTracks) {
            content += `#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID="audio",NAME="${track.name
                }",LANGUAGE="${track.code}",URI="${track.file}",DEFAULT=${track.default ? "YES" : "NO"
                }\n`;
        }
        content += `\n`;
    }
...
async function runAll() {
    logWithTimestamp("🍎 Starting macOS Apple Silicon transcoding job", chalk.magenta);
    ...
    try {
        ...
        logWithTimestamp("✅ FFmpeg detected: " + stdout.split('\n')[0], chalk.green);
    } catch (error) {
        logWithTimestamp("❌ FFmpeg not found or not working. Please install FFmpeg.", chalk.red);
        process.exit(1);
    }
    ...
    logWithTimestamp("📺 Your HLS stream is ready at: " + path.join(outputDir, "master.m3u8"), chalk.cyan);
}
